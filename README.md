...
- When row 1 exceeds the width budget, the working directory and git branch get shortened to make it fit: the longer of the two is shortened first; if still too long, the other is shortened too. The folder drops middle directory segments (`~/Dev/github/proj/leaf` → `~/Dev/.../leaf` → `~/.../leaf`); the branch trims its tail with a 20-char floor so the type/ticket prefix (e.g. `feature/lsc.716962`) survives. Terminal width can't be auto-detected in the statusline spawn — the budget defaults to **200 columns**. Override by setting the `STATUSLINE_MAX_WIDTH` env var (e.g. `STATUSLINE_MAX_WIDTH=160`) before launching Claude Code.
